...
public class PriceController {
    ...
    public List<BigDecimal> selectForDay(@PathVariable String currency) {
        return priceService.selectForDay(currency);
    }

    @GetMapping("/selectForWeek/{currency}")
    public List<BigDecimal> selectForWeek(@PathVariable String currency) {
        return priceService.selectForWeek(currency);
    }

    @GetMapping("/selectForMonth/{currency}")
    public List<BigDecimal> selectForMonth(@PathVariable String currency) {
        return priceService.selectForMonth(currency);
    }

    @GetMapping("/selectFor3Month/{currency}")
    public List<BigDecimal> selectFor3Month(@PathVariable String currency) {
